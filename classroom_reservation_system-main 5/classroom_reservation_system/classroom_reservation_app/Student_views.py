...
def add_reservation2(request, id_bat, date, materiel, nb_pers, duree):
    batiment = Batiment.objects.get(num_batiment=id_bat)
    classe = Salle.objects.filter(id_batiment=id_bat, type=materiel).first()
...
def add_reservation3(request, id_bat, date, materiel, nb_pers, duree, heure_debut):
    batiment = Batiment.objects.get(num_batiment=id_bat)
    sallereservee = Reservation.objects.filter(date=date, idsalle__id_batiment=id_bat, idsalle__type=materiel).values('idsalle')
    classedispo = Salle.objects.filter(id_batiment=id_bat, type=materiel).exclude(idsalle__in=sallereservee)
    reservation = Reservation.objects.filter(date=date)
    return render(request, 'student_templates/add_reservation_template3.html', {"batiment":batiment, "date": date, "materiel": materiel, "nb_pers": nb_pers, "duree": duree, "heure_debut": heure_debut, "classedispo": classedispo, "reservation": reservation})
