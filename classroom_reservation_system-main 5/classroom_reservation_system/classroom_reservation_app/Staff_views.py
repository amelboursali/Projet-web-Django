...
def add_reservation2(request, id_bat, date, materiel, nb_pers, duree):
    batiment = Batiment.objects.get(num_batiment=id_bat)
    classes = Salle.objects.filter(id_batiment=id_bat, type=materiel)
    reservations = Reservation.objects.filter(idsalle__in=classes, date=date)
    return render(request, 'staff_templates/add_reservation_template2.html',{"batiment": batiment, "date": date, "materiel": materiel, "nb_pers": nb_pers, "duree": duree, "classes": classes, "reservations": reservations})
